...
def plot_rts(df):
    ...
    plt.ylabel('Reaction Times (msec)')
...
def get_ems(df, trial):
    ...
    indexes = np.logical_and( df['em_time'][trial] > df['stimon'][trial], df['em_time'][trial] < df['response'][trial] )
    t = np.trim_zeros(indexes * df['em_time'][trial])
    h = np.trim_zeros(indexes * df['em_horiz'][trial])
    v = np.trim_zeros(indexes * df['em_vert'][trial])
...
def plot_ems_and_target(df, trial):
    ...
    plt.plot(t, h, 'r', t, v, 'g', t)
    plt.hlines(df['targ_x'][trial], np.min(t), np.max(t), color='r')
    plt.hlines(df['targ_y'][trial], np.min(t), np.max(t), color='g')
    plt.xlim(np.min(t)-40, np.max(t)+40)
    plt.ylim(-10, 10)
    plt.legend(('Horizontal', 'Vertical'), title='', loc='upper right')
    plt.xlabel('Time [ms]')
    plt.ylabel('Position (degrees visual angle)')
    plt.title('Eye Movements for Trial ' + np.str(trial) )
...
def get_rate(spk_times, start, stop):
    ...
    spk_count = np.count_nonzero(np.logical_and(spk_times>start,spk_times<=stop))
    spk_time_range = (stop-start)/1000.0 
    
    # Remember that rate should be in the units spikes/sec
    # but start and stop are in msec (.001 sec)

    rate = ( spk_count / spk_time_range )
